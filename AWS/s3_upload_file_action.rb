...
# This action enables easy file uploads to S3 buckets, which is essential for
# storing AI-generated content or processing results in cloud storage.
#
...
# in S3 for later retrieval or distribution.
  ...
  def upload_file
    File.open(@file_path, 'rb') do |file|
      ...
      if response.etag
        ...
        error_message = 'Upload completed but no ETag received from S3'
        Sublayer.configuration.logger.log(:error, error_message)
        raise StandardError, error_message
      end
    end
